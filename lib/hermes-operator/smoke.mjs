...
export function verifyHermesOperatorSmoke(input) {
  ...
  if (!input.triggerAttempted) failures.push('operator_trigger_not_attempted')
  if (!input.triggerOk) failures.push('operator_trigger_failed')
  if (!input.runAdvanced) failures.push('operator_run_not_advanced')
  ...
  if (missing(input.businessAlertCount)) failures.push('operator_business_alert_missing')
  if (missing(input.briefCount)) failures.push('operator_brief_missing')
  if (invalidCount(input.blockedByPolicyCount)) failures.push('operator_policy_accounting_missing')
  if (invalidCount(input.acceptedFollowUpScanCount)) failures.push('operator_follow_up_acceptance_missing')
  if (invalidCount(input.acceptedProspectCount)) failures.push('operator_prospect_acceptance_missing')
  if (invalidCount(input.acceptedDevopsCount)) failures.push('operator_devops_acceptance_missing')
