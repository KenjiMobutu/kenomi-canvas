...
describe('parseAgentOutput', () => {
  ...
  it('parse une sortie Prospect JSON structurée', () => {
    const parsed = parseAgentOutput(
      ...
      JSON.stringify({
        ...
        contact_email: 'lina@acme.test',
        source_url: 'https://www.upwork.com/freelancers/~lina',
        score: 88,
        band: 'hot',
  ...
  it('valide une sortie DevOps structurée', () => {
    const parsed = parseAgentOutput(
      'devops',
      JSON.stringify({
    ...
    expect(parsed).toMatchObject({
      global_status: 'degraded',
      services: [{ id: 'ollama', status: 'down', severity: 'high' }],
    })
  })
