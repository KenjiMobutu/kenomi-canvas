...
import {
  ...
  accent, accent2, emerald, rose, amber,
} from '@/lib/ck-vars'
...
export default function ApiKeysPage() {
  ...
  async function create() {
    ...
    const { error } = await supabase.from('api_keys').insert({
      user_id: user.id, name: name.trim(), key_prefix: k.slice(0, 12), key_hash: hash,
    })
    setCreating(false)
  ...
  return (
    <CkShell breadcrumb="System / API Keys" title="API Keys" subtitle={`${keys.length} clé${keys.length !== 1 ? 's' : ''} active${keys.length !== 1 ? 's' : ''}`} actions={createAction}>

      {/* Revealed key banner */}
      {reveal && (
        <div style={{
          marginBottom: 20, padding: 20, borderRadius: 14,
          background: `linear-gradient(135deg, ${accent}18, ${accent2}12)`,
          border: `1.5px solid ${accent}55`,
          boxShadow: `0 0 30px ${accent}20`,
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: accent, letterSpacing: '.18em', textTransform: 'uppercase', fontWeight: 800 }}>
          ...
          <div style={{
            ...
            <span style={{ flex: 1, fontFamily: 'var(--font-mono)', fontSize: 12.5, color: text, wordBreak: 'break-all', letterSpacing: '.04em' }}>
              {reveal}
            </span>
            <button onClick={() => { navigator.clipboard.writeText(reveal); toast.success('Clé copiée') }} style={{
          ...
          <button onClick={() => setReveal(null)} style={{
            ...
            fontFamily: 'var(--font-mono)', fontSize: 10, color: muted2, letterSpacing: '.1em',
          }}>Fermer ↑</button>
        </div>
      )}
      ...
      <div style={{ background: surface, border: `1px solid ${line}`, borderRadius: 14, overflow: 'hidden' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: muted2, marginTop: 2, letterSpacing: '.04em' }}>
                  {k.key_prefix}••••••••••••
                </div>
              </div>
            </div>
            ...
            <button onClick={() => del(k.id)} style={{
              ...
              color: muted2, padding: 4, borderRadius: 4, display: 'grid', placeItems: 'center',
              transition: 'color .1s',
            }}
        ...
        {keys.length === 0 && (
          <div style={{ padding: '64px 24px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 700, color: text, marginBottom: 6 }}>Aucune clé API</p>
            <p style={{ fontSize: 12, color: muted2 }}>Créez une clé pour accéder à l'API Kenomi depuis vos outils externes.</p>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: amber + '20', border: `1px solid ${amber}30`,
          display: 'grid', placeItems: 'center',
          fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 14, color: amber,
        }}>⚡</div>
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: text }}>Utilisation des clés API</div>
          <div style={{ fontSize: 11, color: muted, marginTop: 2 }}>
            Ajoutez l'en-tête <code style={{ fontFamily: 'var(--font-mono)', background: surface2, padding: '1px 5px', borderRadius: 3, color: accent }}>Authorization: Bearer ken_…</code> à chaque requête vers <code style={{ fontFamily: 'var(--font-mono)', background: surface2, padding: '1px 5px', borderRadius: 3, color: accent }}>lab.kenomi.eu/api/v1/</code>
          </div>
        </div>
      </div>
    </CkShell>
