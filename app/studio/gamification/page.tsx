...
function RarityPill({ rarity }: { rarity: Achievement['rarity'] }) {
  ...
  return (
    ...
    >
      <span
        style={{ width: 5, height: 5, borderRadius: '50%', background: r.color, flexShrink: 0 }}
      />
      {r.label}
    </span>
  )
      ...
      {u ? (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {claimed ? (
            ...
            <button
              onClick={() => onClaim(a.id, a.xp)}
              style={{
                padding: '5px 12px',
                borderRadius: 999,
                background: `linear-gradient(90deg, ${a.color}, ${accent2})`,
                color: '#0b0d12',
                border: 'none',
                cursor: 'pointer',
                fontFamily: 'var(--font-mono)',
  ...
  return (
    ...
    >
      <svg width="100" height="100" viewBox="0 0 100 100">
        ...
        <path
          d="M50 4 L92 18 V50 C92 73 73 90 50 96 C27 90 8 73 8 50 V18 Z"
          fill={`url(#lb-${level}-${highlight})`}
          stroke={highlight ? 'rgba(255,255,255,.4)' : 'rgba(255,255,255,.1)'}
          strokeWidth="1.5"
        />
        <path
          d="M50 10 L86 22 V50 C86 70 70 84 50 90 C30 84 14 70 14 50 V22 Z"
          fill="rgba(0,0,0,.35)"
        />
      </svg>
      <div
...
function StatDelta({
  ...
  stat: { label: string; from: number; to: number; suffix: string; color: string; invert?: boolean }
  active: boolean
  delay: number
}) {
  return (
    ...
    >
      ...
      >
        {stat.invert ? '↓' : '↑'}{' '}
        {Math.abs(stat.to - stat.from).toFixed(stat.suffix === 's' ? 1 : 0)}
        {stat.suffix}
      </div>
    </div>
  )
...
function LevelUpTab({
  ...
  const skill = {
    ...
    desc: 'Auto-A/B prix selon CPC, churn et signal payeur',
    badge: '✦',
  }
            ...
            {agentLevels.map((al) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      style={{ width: `${al.xpBar * 100}%`, height: '100%', background: ag.color }}
                    />
                  </div>
                </div>
              )
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {[0, 1, 2].map((i) => (
              <svg
                ...
                viewBox="0 0 14 14"
                style={{
                  animation: 'lu-arrow 1s ease-in-out infinite',
                  animationDelay: `${i * 0.15}s`,
                }}
              >
                <path
                  d="M2 7 H10 M7 3 L11 7 L7 11"
                  stroke={agent.color}
                  strokeWidth="2"
                  fill="none"
                  strokeLinecap="round"
...
function AchievementsTab({
  ...
  const claimed = useMemo(
    () => new Set([...claimedIds, ...optimisticClaimed]),
    [claimedIds, optimisticClaimed]
  )
  ...
  async function handleClaim(id: string, xp: number) {
    ...
    if (user) {
      ...
      const { error } = await supabase
        ...
        .upsert({ user_id: user.id, achievement_id: id, xp })
      ...
      <div style={{ padding: isMobile ? '16px 12px 40px' : '24px 32px 40px', maxWidth: 1400 }}>
        <div
          style={{
            ...
            border: `1px solid ${line}`,
            marginBottom: 24,
            animation: 'ach-shimmer 1.8s linear infinite',
            backgroundImage: `linear-gradient(110deg, ${surface} 30%, ${surface2} 50%, ${surface} 70%)`,
            backgroundSize: '200% 100%',
          }}
        />
        <div
  ...
  const justUnlocked = achievements.find((a) => a.unlocked && !claimed.has(a.id)) ??
    achievements.find((a) => !a.unlocked) ??
    achievements[0] ?? { ...ACHIEVEMENTS_META[0], unlocked: false, pct: 0 }
          ...
          >
            ...
            <span
              style={{
                ...
                padding: '4px 10px',
                borderRadius: 999,
                background: justUnlocked.color + '1e',
                color: justUnlocked.color,
                letterSpacing: '.14em',
                fontWeight: 700,
              }}
          ...
          {justUnlocked.unlocked ? (
            <button
              ...
              style={{
                padding: '12px 22px',
                borderRadius: 999,
                background: claimed.has(justUnlocked.id)
                  ? surface2
                  : `linear-gradient(90deg, ${justUnlocked.color}, ${accent2})`,
                ...
                fontSize: 12.5,
                letterSpacing: '.06em',
                boxShadow: claimed.has(justUnlocked.id)
                  ? 'none'
                  : `0 6px 24px ${justUnlocked.color}55`,
              }}
            >
              {claimed.has(justUnlocked.id) ? '✓ Réclamé' : `RÉCLAMER · +${justUnlocked.xp} XP`}
            </button>
          ) : (
            <div
              style={{
            ...
            >
              ...
              >
                <div
                  style={{
                    width: `${justUnlocked.pct}%`,
                    height: '100%',
                    background: `linear-gradient(90deg, ${justUnlocked.color}, ${accent2})`,
...
const CHATTER_DATA = [
  {
    ...
    to: 'validation',
    type: 'signal',
    text: "r/solopreneur spike +312% — 'AI inbox triage'",
  },
  {
    from: 'validation',
    to: 'decision',
    type: 'score',
    text: 'HR Ops Inbox · score 78 / CPC 0.42€',
  },
  ...
  { from: 'scout', to: 'marketing', type: 'trend', text: "TikTok hook — 'AI bookkeeper'" },
  {
    ...
    to: 'builder',
    type: 'spec',
    text: 'Niche: bilingual freelancer invoicing',
  },
]
const VENTURES_QUEST = [
  {
    ...
    mrr: 2800,
    cac: 14,
    agents: ['analytics', 'marketing', 'payment', 'decision'],
  },
  {
    id: 'cfo',
    name: 'Solo CFO Copilot',
    status: 'Continue',
    score: 84,
    mrr: 620,
    cac: 21,
    agents: ['validation', 'marketing', 'analytics'],
  },
  {
    id: 'ops',
    name: 'HR Ops Inbox',
    ...
    mrr: 0,
    cac: 0,
    agents: ['validation', 'marketing', 'builder'],
  },
  {
    id: 'legal',
    ...
    mrr: 310,
    cac: 39,
    agents: ['builder', 'payment', 'marketing'],
  },
  {
    id: 'crm',
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: 60 }).map((_, i) => {
        ...
        const r1 = CST_R + 60,
          r2 = CST_R + (i % 5 === 0 ? 68 : 64)
        return (
          <line
            key={i}
            x1={CST_CX + Math.cos(a) * r1}
            y1={CST_CY + Math.sin(a) * r1}
            x2={CST_CX + Math.cos(a) * r2}
            y2={CST_CY + Math.sin(a) * r2}
      ...
      <g>
        <animateTransform
          ...
          type="rotate"
          from={`0 ${CST_CX} ${CST_CY}`}
          to={`360 ${CST_CX} ${CST_CY}`}
          dur="8s"
        ...
        return (
          <g key={`${idx}-${beam.text}`}>
            ...
            <circle r={5} fill={fromA.color} filter="url(#cstSoftGlow)">
              <animate
                attributeName="cx"
                from={String(Math.round(fromA.x))}
                to={String(Math.round(toA.x))}
                dur="1.5s"
                repeatCount="indefinite"
              />
              <animate
                attributeName="cy"
                from={String(Math.round(fromA.y))}
                to={String(Math.round(toA.y))}
                dur="1.5s"
                repeatCount="indefinite"
              />
            </circle>
          ...
          <div style={{ width: 38, height: 38, position: 'relative' }}>
            <svg width="38" height="38" viewBox="0 0 38 38">
              ...
              <circle
                ...
                stroke={d.color}
                strokeWidth="3"
                strokeDasharray={`${(d.conf / 100) * 94} 94`}
                strokeLinecap="round"
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <div
            style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: muted, letterSpacing: 1 }}
          >
            {agent.code} · {agent.model.toUpperCase()}
          </div>
        </div>
      </div>
        ...
        >
          <span
            style={{
              ...
              padding: '3px 8px',
              borderRadius: 4,
              background: (STATUS_COLOR[v.status] ?? accent) + '22',
              color: STATUS_COLOR[v.status] ?? accent,
              letterSpacing: 1.5,
              fontWeight: 700,
              textTransform: 'uppercase' as const,
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${ag.color}66`,
                    display: 'grid',
                    placeItems: 'center',
                    fontFamily: 'var(--font-display)',
                    fontSize: 11,
            ...
            return (
              <tr key={fromId}>
                ...
                {AGENT_IDS.map((toId) => {
                  ...
                  const alpha = Math.round(15 + intensity * 60)
                    .toString(16)
                    .padStart(2, '0')
                  return (
                    <td
                      ...
                      {isDiag ? (
                        ...
                        <span
                          style={{
                            ...
                            color: intensity > 0.4 ? fromAg.color : muted,
                          }}
                        >
                          {vol}
                        </span>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          style={{
            padding: isMobile ? '12px 4px' : '16px 20px',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
          }}
      ...
      >
        {AGENTS_DATA.map((agent) => {
          ...
          return (
            <AgentCard key={agent.id} agent={agent} level={al?.level ?? 0} xpBar={al?.xpBar ?? 0} />
          )
        })}
      </div>
      ...
      >
        ...
        <div
          style={{
            ...
            alignItems: 'center',
            whiteSpace: 'nowrap',
            animation: 'const-ticker 32s linear infinite',
          }}
        >
          {[...CHATTER_DATA, ...CHATTER_DATA].map((c, i) => {
            ...
            return (
              <span
                key={i}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '0 18px' }}
              >
                <span
                  style={{
                    fontFamily: 'var(--font-mono)',
...
export default function GamificationPage() {
  ...
  const { achievements, agentLevels, lastLevelUp, userLevel, userXP, loading, claimed, refetch } =
    useGamification()
    ...
    >
      ...
      {tab === 'levelup' ? (
        <LevelUpTab
          ...
          onSwitchTab={() => setTab('achievements')}
        />
      ) : tab === 'constellation' ? (
